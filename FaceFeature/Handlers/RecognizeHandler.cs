...
namespace FaceFeature.Handlers
{
    ...
    public static class RecognizeHandler
    {
        ...
        /// <summary>
        /// 处理图片识别请求：上传原始图片二进制数据，检测人脸并提取特征后与指定分组内的人物逐一比对
        /// </summary>
        ...
        public static async IAsyncEnumerable<FaceRecognition> HandleImageAsync(
            ...
            [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            var detections = DetectHandler.HandleImageAsync(request, detectService, logger, cancellationToken);
        ...
        /// <summary>
        /// 处理图片 URL 识别请求：通过图片 URL 下载后检测
        /// </summary>
        public static async IAsyncEnumerable<FaceRecognition> HandleImageUrlAsync(
            IFaceGroupProvider faceGroupProvider,
        ...
        {
            var persons = await faceGroupProvider.GetPersonsAsync(groupId, cancellationToken);
            var detections = DetectHandler.HandleImageUrlAsync(urlRequest, detectService, logger, httpClient, cancellationToken);
            ...
            [EnumeratorCancellation] CancellationToken cancellationToken)
        {
            await foreach (var detection in detections.WithCancellation(cancellationToken))
            {
                ...
                if (bestMatch != null)
                {
                    yield return new FaceRecognition(
                        bestMatch.Person.Id,
