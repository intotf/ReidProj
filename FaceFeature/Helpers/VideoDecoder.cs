...
internal static class VideoDecoder
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                frameCount++;
                Log.VideoDecodeCompleted(logger, ToFfmpegFormat(codec), frameCount, sw.Elapsed.TotalMilliseconds);
                yield return image;
            }
        }
        finally
        ...
        sw.Stop();
        Log.VideoDecodeAllCompleted(logger, frameCount, ToFfmpegFormat(codec), sw.Elapsed.TotalMilliseconds);
    }
    ...
    /// <summary>将视频流 pipe 到 ffmpeg 的 stdin 并关闭</summary>
    ...
    /// <summary>从 ffmpeg stdout 中流式读取所有 BMP 帧并逐帧解码</summary>
