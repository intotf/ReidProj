...
public sealed class FaceDetector : IDisposable
{
    ...
    /// <summary>
    /// 检测裁剪区域内置信度最高的人脸（适用于从人物框中检测人脸）
    /// </summary>
    /// <param name="image">裁剪后的人物区域 RGB 图像</param>
    /// <param name="offsetX">该裁剪区域在父图中的左上角 X 坐标</param>
    /// <param name="offsetY">该裁剪区域在父图中的左上角 Y 坐标</param>
    /// <returns>人脸检测结果（坐标已偏移到父图空间），未检测到时返回 null</returns>
